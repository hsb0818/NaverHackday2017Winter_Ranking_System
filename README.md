...
- 댓글 수가 아닌, 댓글 '증감'수 이므로 서버 실행 이후의 모든 데이터를 저장한다.
...
## 추가 고려사항
 만약 서버가 새로 개발되어 적용하게 된 상황이라면, 이전의 댓글들에 대한 데이터를 분석할 필요가 있을 것이다. 현재는 [분 단위의 현재시간 - 1분] 간격으로 현재 시간을 기준으로 랭킹을 갱신하는데, 이 알고리즘을 그대로 활용하여 Cron을 이용해 Background에서 또 하나의 Daemon을 돌리면 해결된다. 단지 Input으로 원하는 댓글 증감 분석 기간을 준다면 ___[(10 ~ 15) * 원하는 분석 기간]초___ 의 시간이 걸리며 해결될 것이다.
